      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-4xl max-h-[90vh] bg-white dark:bg-emberOrange rounded-3xl shadow-2xl overflow-hidden flex flex-col"
          >
            <div className={`shrink-0 ${accent.modalHeader} p-6 text-espresso`}>
              <div className="flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold mb-1">Work Experience</h2>
                  <p className="text-espresso/80 text-sm">Where I&apos;ve built things</p>
                </div>
                <button
                  onClick={onClose}
                  className="p-2 rounded-lg bg-espresso/10 hover:bg-espresso/20 transition-colors"
                  aria-label="Close"
                >
                  <X className="w-5 h-5" />
                </button>
                ...
                {experienceModal.map((exp, index) => (
                  ...
                  >
                    <div className="flex flex-col md:flex-row md:items-start md:justify-between mb-4">
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          <div className={`rounded-xl flex items-center justify-center w-12 h-12 shrink-0 ${accent.iconBg}`}>
                            <Image
                              src={exp.companyLogo}
                              alt={exp.company}
                              width={32}
                        ...
                        <div className="flex flex-wrap gap-4 text-sm text-gray-500 dark:text-gray-400 mb-3">
                          ...
                          <span className={`px-2 py-0.5 rounded-full border text-xs ${accent.chip}`}>{exp.type}</span>
                        </div>
                    ...
                    <div className="mb-4 p-4 rounded-xl bg-white/70 dark:bg-white/[0.02] border border-gray-200/60 dark:border-white/5">
                      <h4 className="font-medium text-gray-900 dark:text-gray-100 mb-1 text-sm">About {exp.company}</h4>
